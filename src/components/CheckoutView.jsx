    ...
    <div className="min-h-screen bg-slate-50 font-sans pb-32 antialiased">
      ...
      <div className="px-5 mt-6 flex flex-col gap-6">
        <div className="bg-white p-5 rounded-2xl shadow-sm border border-slate-200">
          ...
          {deliveryMethod === 'delivery' && (
            <div className="mt-4 flex gap-3 animate-fade-in-up">
              <div className="flex-1"><label className="text-[11px] font-bold text-slate-500 mb-1 block">Lantai</label><input type="text" placeholder="Cth: Lt. 5" value={floor} onChange={(e) => setFloor(e.target.value)} className="w-full bg-slate-50 border border-slate-200 px-3 py-2 rounded-lg text-sm font-semibold outline-none focus:border-blue-700" /></div>
              <div className="flex-1"><label className="text-[11px] font-bold text-slate-500 mb-1 block">No. Meja</label><input type="text" placeholder="Cth: M-12" value={desk} onChange={(e) => setDesk(e.target.value)} className="w-full bg-slate-50 border border-slate-200 px-3 py-2 rounded-lg text-sm font-semibold outline-none focus:border-blue-700" /></div>
            </div>
          )}
        </div>
        ...
        <div className="bg-white p-5 rounded-2xl shadow-sm border border-slate-200">
          ...
          <input type="text" placeholder="Ada catatan? (Cth: Esnya dipisah ya)" value={notes} onChange={(e) => setNotes(e.target.value)} className="w-full mt-5 bg-slate-50 border border-slate-200 px-4 py-2.5 rounded-xl text-sm font-medium outline-none focus:border-blue-700" />
        </div>
      ...
      <div className="fixed bottom-0 left-0 right-0 p-5 bg-white border-t border-slate-200 max-w-md mx-auto z-30">
        <button onClick={handleOrder} className="w-full bg-blue-900 text-white font-extrabold py-3.5 rounded-xl shadow-lg shadow-blue-900/30 hover:bg-blue-800 active:scale-[0.98] transition-all flex justify-between items-center px-6">
